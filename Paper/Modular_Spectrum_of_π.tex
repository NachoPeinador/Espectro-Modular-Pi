...
This structure allows for reformulating the Euler Product for $\pi$, isolating the contribution of primes 2 and 3. Starting from Euler's identity $\zeta(2) = \pi^2/6$ and extracting the factors corresponding to $p=2$ and $p=3$, we obtain:
\begin{equation}
...
\begin{remark}[The Optimal Ternary Base]
The normalization factor $9 = 3^2$ that naturally emerges here is not an isolated algebraic artifact. As we will see in the following section, this primorial square reappears in the factorization of high-level geometric bases (such as the invariant $396 = 2^2 \times 3^2 \times 11$), revealing a deep self-similarity between the arithmetic of the elementary substrate and complex multiplication modular forms.
\end{remark}
% --------------------------------------------------------
