\documentclass{mcom-l} % Cambiado a clase oficial de la AMS y removidas opciones de tamaño 

\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage[english]{babel}
% amsmath y amsthm se eliminan por ser cargados automáticamente por la clase amsart/proc-l 
\usepackage{amssymb} % Cargado para símbolos adicionales
\usepackage{graphicx} % Recomendado para inclusión de gráficos
\usepackage{hyperref}
\usepackage{booktabs}
\usepackage{float}
\usepackage{mathtools} % Paquete compatible para mejoras de visualización
\usepackage{physics}
\usepackage{cleveref}
\usepackage{enumitem}
...
% Se elimina el paquete geometry y configuraciones manuales de márgenes por alterar dimensiones nativas
...
% All newtheorem specifications included in the preamble
\theoremstyle{plain} % Extra space above/below, text in italic
\newtheorem{theorem}{Theorem}[section]
\newtheorem{proposition}[theorem]{Proposition}
\newtheorem{lemma}[theorem]{Lemma}
\newtheorem{corollary}[theorem]{Corollary}

\theoremstyle{definition} % Space above/below, upright text
\newtheorem{definition}[theorem]{Definition}

\theoremstyle{remark} % No extra space, upright text
\newtheorem{remark}[theorem]{Remark}

\begin{document}

% --- AMS TOP MATTER (Ingresado estrictamente después de \begin{document} y en orden)

% Título corto entre [] y Título completo + Subtítulo separado por dos puntos entre {}
\title[Polyphase Isomorphism]{Polyphase isomorphism between modular arithmetic and multirate digital signal processing: With formal verification in Lean 4 and computational validation}
...
% Requerido: Clasificación temática completa de la MSC 2020
\subjclass[2020]{Primary 11A07, 94A12; Secondary 65Y05, 68V15}

% Opcional: Solo primera palabra y nombres propios capitalizados
\keywords{Polyphase decomposition, modular arithmetic, multirate signal processing, perfect reconstruction filter banks, formal verification, Lean 4, Mathlib, parallel architectures, Leibniz series, Euler product.}

% El abstract debe colocarse obligatoriamente antes de \maketitle
\begin{abstract}
We establish a formal mathematical isomorphism between the modular decomposition of integer-indexed series in number theory and the polyphase decomposition of discrete-time signals in multirate digital signal processing. Under this isomorphism, the projection of a series \(\sum a_n\) onto the congruence classes modulo \(m\) is exactly equivalent to the decimation of the discrete signal \(x[n] = a_n\) by a factor \(M=m\) and the extraction of its polyphase components. We prove that this correspondence is exact, independent of the specific sequence or modulus, and guarantees perfect reconstruction without aliasing. The theorem is validated through two complementary approaches: (i) a formal verification of the underlying algebraic and phase properties using the Lean 4 theorem prover and the Mathlib mathematical library, which certifies the modular involution, the unit group isomorphism \((\mathbb{Z}/6\mathbb{Z})^{\times} \cong \mathbb{Z}_2\), the anti-unitary phase symmetry \(\Delta\phi = \pi\), and the perfect reconstruction property of the polyphase synthesis bank; and (ii) a computational validation via a Google Colab notebook that numerically verifies the isomorphism on the Leibniz series for \(\pi\) and on the modular Euler product, achieving machine-precision agreement. The isomorphism allows the transfer of techniques between the two domains: results from number theory provide new insights into filter bank design and parallel computational architectures, while tools from signal processing offer novel methods for analyzing arithmetic series. We illustrate the practical implications with a Shared-Nothing parallel algorithm for hypergeometric series evaluation, whose theoretical efficiency is derived from the polyphase decomposition.
\end{abstract}

\maketitle % Genera el bloque de título y "top matter" automáticamente
...
The algebraic foundations of the isomorphism have been formally verified using the Lean 4 theorem prover and the Mathlib mathematical library \cite{mathlib}. The verification, available as a companion Colab notebook, certifies the following properties:
...
We have established a formal isomorphism between modular decomposition in number theory and polyphase decomposition in multirate digital signal processing. The isomorphism is exact, general, and independent of the specific sequence or modulus. It has been validated both formally (via Lean 4) and computationally (via Colab), providing a high degree of confidence in its correctness.
...
\textbf{Future work:} Several extensions are natural: (i) extending the isomorphism to non-uniform filter banks; (ii) exploring the connection with the Discrete Fourier Transform and Dirichlet characters; (iii) applying the isomorphism to \(L\)-functions; and (iv) extending the Shared-Nothing architecture---whose theoretical foundations are established here and whose practical implementation and benchmarking are presented in the companion article \cite{peinador_modular}---to FPGAs or ASICs for high-performance series evaluation.
